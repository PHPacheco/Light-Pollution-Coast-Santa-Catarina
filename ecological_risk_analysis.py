...
def summarize_metrics(city, classes, light, bottom_light, habitat, fauna, risk, pixel_m):
    rows = []
...
def write_result_report(rows):
    ...
    for city, city_rows in by_city.items():
        lines.append(f"## {city}")
        lines.append("")
        lines.append("| Buffer | Area agua km2 | Luz media | Luz fundo | Habitat | Fauna | Risco | Area alto risco km2 |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|")
        for row in city_rows:
            lines.append(
                "| {buffer} | {water_area_km2:.2f} | {radiance_mean_relative:.3f} | "
                "{bottom_light_mean_relative:.3f} | {habitat_proxy_mean:.3f} | "
